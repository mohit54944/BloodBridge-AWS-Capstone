...
def check_low_stock():
    ...
    if low_types:
        send_notification("Low Blood Stock Alert", f"Critical levels detected: {', '.join(low_types)}")
...
def is_eligible(username):
    ...
    if not last_date_str or last_date_str == "": return True, 0
...
def signup():
    if request.method == 'POST':
        ...
        send_notification("New User Signup", f"User {username} has joined BloodBridge.")
...
def admin_signup():
    if request.method == 'POST':
        username = request.form['username']
        admin_table.put_item(Item={'username': username, 'password': request.form['password']})
...
def user_dashboard():
    ...
    inv = {i['blood_type']: i['quantity'] for i in inventory_table.scan()['Items']}
...
def request_blood():
    ...
    if request.method == 'POST':
        ...
        send_notification("New Request", f"Blood Type {request.form['blood_type']} requested by {session['username']}.")
        return redirect(url_for('user_dashboard'))
...
def donate_to_request(req_id):
    ...
    inventory_table.put_item(Item={'blood_type': bt, 'quantity': current_qty + qty})
    ...
    flash(f"Thank you! {qty} units of {bt} have been added to stock.", "success")
    return redirect(url_for('user_dashboard'))
...
def admin_dashboard():
    ...
    if request.method == 'POST':
        for bt in ['O+', 'O-', 'A+', 'A-', 'B+', 'B-', 'AB+', 'AB-']:
            ...
            if val is not None:
                inventory_table.put_item(Item={'blood_type': bt, 'quantity': int(val)})
        check_low_stock()
...
def fulfill_request(req_id):
    ...
    if current_qty >= qty_needed:
        inventory_table.put_item(Item={'blood_type': bt, 'quantity': current_qty - qty_needed})
        requests_table.update_item(Key={'id': req_id}, UpdateExpression="set #s = :v", 
